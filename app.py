...
class MotionEffectsProcessor:
    ...
    def spawn_star_particles_from_body(self, lms, frame_w: int, frame_h: int, move_dx: float, move_dy: float, speed: float) -> None:
        ...
        # Randomly distribute stars over body points
        visible_indices = []
        for lm_idx in self.body_landmarks:
            if lm_idx < len(lms) and lms[lm_idx].visibility > 0.4:
                visible_indices.append(lm_idx)

        if not visible_indices:
            return

        for _ in range(total_stars):
            lm_idx = random.choice(visible_indices)
            lm = lms[lm_idx]
            ...
            px = px_orig + (-dir_x * offset_dist) + random.uniform(-12, 12)
            py = py_orig + (-dir_y * offset_dist) + random.uniform(-12, 12)
